...
<body id="page-top">
  ...
  $pengeluaran_hari_ini = mysqli_query($koneksi, "SELECT jumlah FROM cashinout where tanggal = CURDATE() AND jenis='Pengeluaran'");
  $pengeluaran_hari_ini = mysqli_fetch_array($pengeluaran_hari_ini);

  $pemasukan_hari_ini = mysqli_query($koneksi, "SELECT jumlah FROM cashinout where tanggal = CURDATE() AND jenis='Pemasukkan'");
  $pemasukan_hari_ini = mysqli_fetch_array($pemasukan_hari_ini);




  $grafik_bulanan_pendapatan = mysqli_query($koneksi, "SELECT SUM(jumlah) as total, tanggal FROM cashinout WHERE jenis='Pemasukkan' GROUP BY DATE(tanggal) ORDER BY tanggal");
  while ($grafik_masuk = mysqli_fetch_array($grafik_bulanan_pendapatan)) {
    $tanggal_masuk[] = tanggalIndonesia($grafik_masuk['tanggal']);
    $jumlah_masuk[] = (float) $grafik_masuk['total'];
  }
  //  $grafik_masuk = mysqli_fetch_array($grafik_bulanan_pendapatan);

  $grafik_bulanan_pengeluaran = mysqli_query($koneksi, "SELECT SUM(jumlah) as total, tanggal FROM cashinout WHERE jenis='Pengeluaran' GROUP BY DATE(tanggal) ORDER BY tanggal");
  while ($grafik_keluar = mysqli_fetch_array($grafik_bulanan_pengeluaran)) {
    $tanggal_keluar[] = tanggalIndonesia($grafik_keluar['tanggal']);
    $jumlah_keluar[] = (float) $grafik_keluar['total'];
  }
...
//   $tujuhhari = mysqli_fetch_array($tujuhhari);
  ...
  <div id="content">
    ...
    <nav class="navbar navbar-expand navbar-dark bg-light topbar mb-4 static-top shadow">
      ...
      <!-- Topbar Search -->
      <h4> Selamat Datang, <?= ($_SESSION['nama_pengguna']); ?></h4>
        ...
        <div class="col-xl-3 col-md-6 mb-4">
          <div class="card border-left-info shadow h-100 py-2">
            <div class="card-body">
              <div class="row no-gutters align-items-center">
                <div class="col mr-2">
                  ...
                  <div class="h6 mb-0 mr-3 font-weight-bold text-gray-800">Rp.<?= number_format($uang, 0, ',', '.'); ?></div>
                </div>
                <div class="col-auto">
            ...
            <div class="col">
              ...
              <!-- <div class="progress-bar bg-<?= $warna ?>" role="progressbar" style="width: 100%" aria-valuenow="<?= $value ?>" aria-valuemin="0" aria-valuemax="100"></div> -->
              <!-- </div> -->
            </div>
          </div>
        </div>
      ...
      <div class="row">
        ...
        <div class="col-xl-8 col-lg-7">
          <div class="card shadow mb-4">
            ...
            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
              <h6 class="m-0 font-weight-bold text-warning text-primary">Pendapatan Terakhir</h6>
        ...
        <div class="col-xl-4 col-lg-5">
          <div class="card shadow mb-4">
            <!-- Card Header - Dropdown -->
            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
              <h6 class="m-0 font-weight-bold text-warning text-primary">Perbandingan</h6>
              <div class="dropdown no-arrow">
                <!-- <a class="dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                  <i class="fas fa-ellipsis-v fa-sm fa-fw text-gray-400"></i>
                </a> -->
